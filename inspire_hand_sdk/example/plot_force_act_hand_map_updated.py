    ...
    for name, addr, length, size, var in data_sheet:
        if 'fingerone' in var:  # Pinky
            ...
            if 'tip' in var:
                ...
                w, h = 0.06, 0.12
                x = finger_center_x - w / 2
                y = finger_base_y + finger_length * 0.55
            elif 'palm' in var:
                # Palm side of finger, centered
                w, h = 0.06, 0.12
                x = finger_center_x - w / 2
                y = finger_base_y + finger_length * 0.22
            ...
            if 'tip' in var:
                ...
                x = finger_center_x - w / 2
                y = finger_base_y + finger_length * 0.22
        elif 'fingerfive' in var:  # Thumb (4 stacked regions)
            thumb_center_x = thumb_x + thumb_width / 2
            w = 0.06  # common thumb width
...
def upsample(data, scale=2):
    return zoom(data, zoom=scale, order=1)  # bilinear interpolation

def orient_sensor(var, data):
    """
    Fix sensor matrix orientation so it matches anatomical visualization.
    """
    # Finger sensors: flip along horizontal axis
    if 'finger' in var and 'fingerfive' not in var:
        if 'top' in var or 'palm' in var or ('middle' in var or 'mid' in var):
            return np.flipud(data)
        if 'tip' in var:
            return np.flipud(data)

    # Thumb sensors (including mid)
    if 'fingerfive' in var:
        return np.flipud(data)
        
    # Palm sensor: rotate 90 degrees CCW
    if 'palm_touch' in var:
        return np.rot90(data, k=1)

    # Tips usually fine as-is
    return data
            ...
            for name, addr, length, size, var in data_sheet:
                if var in touch_data and var in sensor_images:
                    ...
                    if sensor_matrix is not None and sensor_matrix.size > 0:
                        ...
                        relative_data = np.clip(relative_data, 0.0, max_raw_data)
