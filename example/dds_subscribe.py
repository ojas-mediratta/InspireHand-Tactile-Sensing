...
class DDSHandler():
    ...
    def read(self):
        with self.data_state_lock and self.data_touch_lock:
            if self.print_timer == 10:
                with open("force.csv","ab") as f:    
                    np.savetxt(f,np.array(self.states['FORCE_ACT']).flatten().reshape(1,-1),delimiter=',',fmt='%.2f')
                with open("touch.csv","ab") as f: 
                    np.savetxt(f,np.array(self.touch['fingerthree_top_touch']).reshape(1,-1),delimiter=',',fmt='%.4f')
                self.print_timer=0
            self.print_timer+=1
